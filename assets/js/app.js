...
function navFor(p){if(p.course||p.lesson)return'academy';if(p.review||p.case||p.artifact||['practice','cases','artifacts','diagnostic'].includes(p.page))return'practice';if(p.term||p.source||p.knowledge||p.page==='knowledge')return'knowledge';if(p.page==='profile')return'profile';return'today'}
...
function renderOnboarding(){setActiveNav('today');$('#app').innerHTML=`<section class="hero" style="max-width:850px;margin:5vh auto"><div class="eyebrow">EA Mastery 5.0</div><h1>Vom ersten Architekturbegriff bis zur Principal-EA-Handlungsfähigkeit.</h1><p class="lead">Diese App misst nicht, wie viel du gelesen hast. Sie führt dich durch Erklären, aktives Erinnern, Behördenfälle und professionelle Architekturartefakte.</p><div class="grid-3 section"><div class="card flat"><strong>1 · Verstehen</strong><p class="small muted">Grundlagen, Begriffe, mentale Modelle.</p></div><div class="card flat"><strong>2 · Anwenden</strong><p class="small muted">Fälle, Trade-offs, Reviews.</p></div><div class="card flat"><strong>3 · Liefern</strong><p class="small muted">Artefakte erstellen und verteidigen.</p></div></div><div class="settings-grid"><label class="field"><span>Tägliche Lernzeit</span><select id="onboardGoal"><option>10</option><option selected>20</option><option>30</option><option>45</option><option>60</option></select></label><label class="field"><span>Lernmodus</span><select id="onboardMode"><option value="guided">Geführt – empfohlen</option><option value="free">Frei erkunden</option></select></label></div><div class="hero-actions"><button id="startAcademy" class="btn btn-primary">Akademie starten</button><button id="startDiagnostic" class="btn btn-soft">Mit Level-Check starten</button></div><p class="small muted">Alle Lerndaten bleiben lokal auf deinem Gerät und können exportiert oder zurückgesetzt werden.</p></section>`;$('#startAcademy').onclick=()=>finishOnboarding(false);$('#startDiagnostic').onclick=()=>finishOnboarding(true)}
...
function openSearch(){$('#searchOverlay').classList.remove('hidden');$('#searchInput').value='';$('#searchResults').innerHTML='<div class="empty">Suche in Lerneinheiten, Lexikon, Fachbüchern und Projektquellen.</div>';setTimeout(()=>$('#searchInput').focus(),30)}
function closeSearch(){$('#searchOverlay').classList.add('hidden')}
function search(q){q=q.trim().toLowerCase();if(q.length<2){$('#searchResults').innerHTML='<div class="empty">Mindestens zwei Zeichen eingeben.</div>';return}let hits=[];for(const x of content.search){const hay=(x.title+' '+x.meta+' '+x.text).toLowerCase();let score=0;if(x.title.toLowerCase().includes(q))score+=20;if(hay.includes(q))score+=5;if(score)hits.push({...x,score})}for(const theme of content.books)for(const b of theme.books){const hay=(b.title+' '+b.author+' '+b.summary).toLowerCase();if(hay.includes(q))hits.push({kind:'book',id:'',title:b.title,meta:`Buch · ${theme.title}`,text:b.summary,score:8})}hits.sort((a,b)=>b.score-a.score);$('#searchResults').innerHTML=hits.slice(0,35).map(h=>{const href=h.kind==='lesson'?`#lesson=${h.id}`:h.kind==='term'?`#term=${h.id}`:h.kind==='source'?`#source=${h.id}`:'#knowledge=books';return `<a class="search-result" href="${href}" onclick="document.querySelector('#searchOverlay').classList.add('hidden')"><span class="badge">${esc(h.meta)}</span><strong style="display:block;margin-top:5px">${esc(h.title)}</strong><p class="small muted">${esc((h.text||'').slice(0,220))}</p></a>`}).join('')||'<div class="empty">Keine Treffer.</div>'}
async function checkVersion(){try{const r=await fetch(`content/version.json?t=${Date.now()}`,{cache:'no-store'}),v=await r.json();const seen=localStorage.getItem('eaMasterySeen');if(seen&&seen!==v.version){$('#updateText').textContent=v.message||v.version;$('#updateBanner').classList.remove('hidden')}else if(!seen)localStorage.setItem('eaMasterySeen',v.version)}catch{}}
...
async function downloadOffline(){try{if(navigator.storage?.persist)await navigator.storage.persist();const cache=await caches.open('ea-mastery-v5.0.0');const core=['./','./index.html','./assets/styles.css','./assets/js/app.js','./assets/js/core/content.js','./assets/js/core/store.js','./assets/js/core/srs.js','./assets/js/core/mastery.js','./assets/js/core/utils.js','./assets/js/views/dashboard.js','./assets/js/views/academy.js','./assets/js/views/lesson.js','./assets/js/views/practice.js','./assets/js/views/knowledge.js','./assets/js/views/profile.js','./content/curriculum/courses.json','./content/curriculum/lessons.json','./content/practice/cards.json','./content/practice/cases.json','./content/knowledge/lexicon.json','./content/knowledge/books.json','./content/knowledge/sources.json','./content/knowledge/search.json'];const urls=[...core,...content.sources.map(s=>'./'+s.path)];let n=0;for(const u of urls){try{await cache.add(u)}catch{}if(++n%10===0)toast(`Offline ${n}/${urls.length}`)}toast('Offline-Paket vollständig')}catch(e){toast('Offline-Paket konnte nicht vollständig geladen werden')}}
