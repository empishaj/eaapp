...
const paths={
  ...
  books:'content/knowledge/books.json',sources:'content/knowledge/sources.json',search:'content/knowledge/search.json',artifacts:'content/practice/artifacts.json',diagrams:'content/visuals/diagrams.json'
};
async function j(path){const r=await fetch(new URL(path,ROOT));if(!r.ok)throw new Error(`${path}: ${r.status}`);return r.json()}
export async function loadContent(){
  const [version,courses,lessons,cards,cases,lexicon,books,sources,search,artifacts,diagrams]=await Promise.all(Object.values(paths).map(j));
  const data={version,courses:courses.courses,stages:courses.stages,lessons:lessons.lessons,cards:cards.cards,cases:cases.cases,lexicon:lexicon.entries||[],books:books.themes||[],sources:sources.sources||[],search:search.items||[],artifactExamples:artifacts.artifacts||[],diagrams:diagrams.diagrams||[]};
